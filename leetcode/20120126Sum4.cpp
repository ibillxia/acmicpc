...
public:
    vector<vector<int> > fourSum(vector<int> &num, int target) {
        ...
        vector<int> vct;
        for(i=0;i<num.size()-1;i++){ // hash for sum of two numbers
        ...
        map<int,bool> mib;
        for(it=vct.begin();it!=vct.end();it++){ // for each sum of two numbers
            s=target-*it;
            if(mib.count(*it)||mib.count(s))continue; // sum already checked
...
int main()
{
    ...
    vector<vector<int> > ans = sol.fourSum(v,-236727523);
    cout<<ans.size()<<endl;
    return 0;
}
